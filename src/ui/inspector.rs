...
impl Inspector {
    ...
    pub fn draw(&self, renderer: &TextRenderer, rect_shader: &Shader, width: f32, height: f32, selected_name: &str, current_pos: Vec3) {
        let panel_h = 160.0;
        let panel_w = 200.0;
        ...
        // Header Text
        renderer.render_text(selected_name, self.x, self.y + panel_h - 28.0, 20.0, Vec3::ONE, width, height);

        // Labels with values
        let val_scale = 18.0;
        let label_x = self.x;
        
        renderer.render_text(&format!("X: {:.2}", current_pos.x), label_x, self.x_dec.y + 8.0, val_scale, Vec3::new(1.0, 0.5, 0.5), width, height);
        self.x_dec.draw(renderer, rect_shader, width, height);
        self.x_inc.draw(renderer, rect_shader, width, height);

        renderer.render_text(&format!("Y: {:.2}", current_pos.y), label_x, self.y_dec.y + 8.0, val_scale, Vec3::new(0.5, 1.0, 0.5), width, height);
        self.y_dec.draw(renderer, rect_shader, width, height);
        self.y_inc.draw(renderer, rect_shader, width, height);

        renderer.render_text(&format!("Z: {:.2}", current_pos.z), label_x, self.z_dec.y + 8.0, val_scale, Vec3::new(0.5, 0.5, 1.0), width, height);
    ...
    pub fn check_clicks(&self, mx: f32, my: f32, height: f32) -> Vec3 {
        ...
        if self.x_dec.is_clicked(mx, my, height) { delta.x -= step; }
        if self.x_inc.is_clicked(mx, my, height) { delta.x += step; }
        
        if self.y_dec.is_clicked(mx, my, height) { delta.y -= step; }
        if self.y_inc.is_clicked(mx, my, height) { delta.y += step; }
        
        if self.z_dec.is_clicked(mx, my, height) { delta.z -= step; }
        if self.z_inc.is_clicked(mx, my, height) { delta.z += step; }
