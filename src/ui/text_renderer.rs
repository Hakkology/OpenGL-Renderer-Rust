...
impl TextRenderer {
    pub fn new(shader: Rc<Shader>) -> Self {
        ...
        unsafe {
            ...
            gl::BufferData(gl::ARRAY_BUFFER, (4 * 4 * std::mem::size_of::<f32>()) as isize, std::ptr::null(), gl::DYNAMIC_DRAW);
    ...
    pub fn render_rect(&self, shader: &Shader, x: f32, y: f32, width: f32, height: f32, color: glam::Vec4, window_width: f32, window_height: f32) {
        ...
        unsafe {
            ...
            gl::BufferSubData(gl::ARRAY_BUFFER, 0, (vertices.len() * std::mem::size_of::<f32>()) as isize, vertices.as_ptr() as *const _);
    ...
    pub fn render_text(&self, text: &str, x: f32, y: f32, scale: f32, color: glam::Vec3, window_width: f32, window_height: f32) {
        let projection = Mat4::orthographic_rh_gl(0.0, window_width, 0.0, window_height, -1.0, 1.0);
        ...
        // Vertices for TRIANGLE_STRIP (Z or N shape)
        // BL, TL, BR, TR or similar. 
        // Let's use: TL, BL, TR, BR
        let vertices: [f32; 16] = [
            x,  y2, 0.0, 0.0, // TL
            x,  y,  0.0, 1.0, // BL
            x2, y2, 1.0, 0.0, // TR
